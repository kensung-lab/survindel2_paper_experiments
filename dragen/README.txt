...
for f in ???????.sv.vcf.gz ; do bcftools view -f PASS $f -Oz -o ${f%vcf.gz}PASS.vcf.gz ; done

for f in ???????.sv.PASS.vcf.gz ; do bcftools view -i "SVTYPE=='DEL'" $f -Oz -o ${f%vcf.gz}DEL.vcf.gz ; done
for f in ???????.sv.PASS.vcf.gz ; do bcftools view -i "SVTYPE=='DUP'" $f -Oz -o ${f%vcf.gz}DUP.vcf.gz ; done
