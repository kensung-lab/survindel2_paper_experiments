...
for f in ???????.sv.vcf.gz ; do bcftools view -i "SVTYPE=='DUP'" $f -Oz -o ${f%vcf.gz}DUP.vcf.gz ; done

for f in ???????.sv.vcf.gz; do sample=`basename $f | cut -d"." -f1`; echo $sample ; 
...
for f in ???????.sv.vcf.gz; do sample=`basename $f | cut -d"." -f1`; echo $sample ; 
    ...
    ../SurVClusterer/compare-ins ../1_data_preparation/by-sample/$sample.INS.vcf.gz $sample.sv.DUP.vcf.gz -T ../1_data_preparation/simpleRepeat.bed -R ../1_data_preparation/GRCh38_full_analysis_set_plus_decoy_hla.fa --report | grep PREC
    echo
